...
function Comment({item}) {
  ...
  return (
    <>
    <div className='flex md:flex-row flex-col my-10 border rounded-lg p-5 bg-slate-100 shadow-sm'>
            {/** SCORE LABEL */}
      ...
      <div className="max-w-[750px] border-l float-right">
      {
              item.replies.length > 0 
              && item?.replies?.map((rep, i) => {
                return <div key={i} className="max-w-[700px] md:pl-0 pl-3 float-right"><Replies item={rep} /></div>
              })
      }
      </div>
